 ...
 * Donnee
 *
 * @ORM\Table(name="t_donneetmp",
 * uniqueConstraints={@ORM\UniqueConstraint(name="UK_search", columns={"horodatage", "cycle", "valeur1", "valeur2", "categorie", "numero_module", "numero_message", "affaire", "numero_localisation"})},
 * indexes={@ORM\Index(name="K_text", columns={"nom_fichier", "erreur"})})
 * @ORM\Entity(repositoryClass="Ipc\ProgBundle\Entity\DonneetmpRepository")
 */
class Donneetmp
{
    /**
   ...
   */
    ...
    public function myInsert($dbh, $liste_donnees) {
        $retour = null;
        $requete = "INSERT INTO t_donneetmp ( erreur, horodatage, cycle, valeur1, valeur2, numero_genre, categorie, numero_module, numero_message, nom_fichier, affaire, programme, numero_localisation ) 
    ...
    public function myUpdateError($dbh) {
		$requete = "UPDATE t_donneetmp SET erreur = '$this->erreur' WHERE id = '$this->id'";
		$retour = $dbh->exec($requete);
 		return($retour);
    }

    public function myDelete($dbh, $liste_donnees) {
       	$requete = "DELETE FROM t_donneetmp WHERE id IN (".$liste_donnees.");";
		$retour = $dbh->exec($requete);
        return($retour);
    }
    ...
    public function SqlGetByError($dbh) {
		$donnees = null;
		$requete = "SELECT SQL_NO_CACHE * FROM t_donneetmp WHERE nom_fichier = '$this->nomFichier' AND erreur = '$this->erreur'";
    ...
    public function SqlGetNb($dbh) {
        $donnees = null;
        $requete = "SELECT SQL_NO_CACHE COUNT(*) FROM t_donneetmp";
        if (($reponse = $dbh->query($requete)) != false) {
            $donnees = $reponse->fetchColumn();
            $reponse->closeCursor();
        }
    ...
    public function SqlGetFiles($dbh, $limit, $offset) {
		$donnees = null;
		$requete = "SELECT SQL_NO_CACHE * FROM ( SELECT nom_fichier,erreur FROM t_donneetmp LIMIT $limit OFFSET $offset) AS Table1 GROUP BY Table1.nom_fichier, Table1.erreur";
		if (($reponse = $dbh->query($requete)) != false) {
            $donnees = $reponse->fetchAll();
            $reponse->closeCursor();
		}
    ...
    //	Retourne un nombre de ligne définie de la table t_donneetmp
    public function SqlGet($dbh, $offset, $limit) {
		$donnees = null;
		$requete = "SELECT SQL_NO_CACHE * FROM t_donneetmp ORDER BY horodatage LIMIT $limit OFFSET $offset";
		if (($reponse = $dbh->query($requete)) != false) {
	    	$donnees = $reponse->fetchAll();
	    	$reponse->closeCursor();
		}
